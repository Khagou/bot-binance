...
const TP_LEVELS = [0.03, 0.07]; // 3% et 7%
...
export async function maybeBuy(state: BotState, candles: Candle[]) {
    ...
    if (trendUp && touchesPullback && cooldownOk && canFinance(state, BASE_ORDER)) {
        ...
        //stats
        state.statsToday.buys += 1; state.statsWeek.buys += 1; state.statsTotal.buys += 1;
        state.statsToday.volumeBought += financed; state.statsWeek.volumeBought += financed; state.statsTotal.volumeBought += financed;
...
export async function handleLotTPs(state: BotState, price: number) {
    for (const lot of state.positions) {
        ...
        if (price >= target) {
            ...
            // stats
            state.statsToday.sells += 1; state.statsWeek.sells += 1; state.statsTotal.sells += 1;
            state.statsToday.tps   += 1; state.statsWeek.tps   += 1; state.statsTotal.tps   += 1;
            state.statsToday.volumeSold += net; state.statsWeek.volumeSold += net; state.statsTotal.volumeSold += net;
            state.statsToday.pnlRealized += profit; state.statsWeek.pnlRealized += profit; state.statsTotal.pnlRealized += profit;
            state.statsToday.skimSaved   += skim;   state.statsWeek.skimSaved   += skim;   state.statsTotal.skimSaved   += skim;
    ...
    if (hasTradingLots) {
        ...
        if (cond) {
            ...
            if (qtyClose > 0) {
                ...
                // stats
                state.statsToday.sells += 1; state.statsWeek.sells += 1; state.statsTotal.sells += 1;
                state.statsToday.stops += 1; state.statsWeek.stops += 1; state.statsTotal.stops += 1;
                state.statsToday.volumeSold += net; state.statsWeek.volumeSold += net; state.statsTotal.volumeSold += net;
